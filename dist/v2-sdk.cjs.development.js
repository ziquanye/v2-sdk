...
var Pair = /*#__PURE__*/function () {
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount) {
    ...
    var numerator = JSBI.multiply(inputAmountWithFee, outputReserve.quotient); //分子

    var denominator = JSBI.add(JSBI.multiply(inputReserve.quotient, _1000), inputAmountWithFee); //分母
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount) {
    ...
    var numerator = JSBI.multiply(JSBI.multiply(inputReserve.quotient, outputAmount.quotient), _1000); //分子

    var denominator = JSBI.multiply(JSBI.subtract(outputReserve.quotient, outputAmount.quotient), _997); //分母

    var inputAmount = sdkCore.CurrencyAmount.fromRawAmount(outputAmount.currency.equals(this.token0) ? this.token1 : this.token0, JSBI.add(JSBI.divide(numerator, denominator), ONE) // JSBI.divide(分子，分母) 分子除以分母的表达式
    );
